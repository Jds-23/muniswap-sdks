...
import { mockCreatePublicClient, mockParseEventLogs } from '../__mocks__/viem'

vi.mock('viem', () => import('../__mocks__/viem'))

const { RpcClient, createRpcClient } = await import('../../src/rpc/client')
const { BlockNotFoundError, NetworkError } = await import('../../src/types')

describe('RpcClient', () => {
  let mockClient: any
  let mockGetBlock: Mock
  let mockGetTransactionReceipt: Mock
  ...
  describe('getBlock', () => {
    ...
    it('should throw BlockNotFoundError when block does not exist', async () => {
      ...
      await expect(client.getBlock(999999)).rejects.toThrow(BlockNotFoundError)
    })

    it('should throw BlockNotFoundError when block is null', async () => {
      mockGetBlock.mockResolvedValue(null)

      await expect(client.getBlock('latest')).rejects.toThrow(BlockNotFoundError)
    })
  })
  ...
  describe('getTransactionReceipt', () => {
    ...
    it('should throw error when receipt is not found', async () => {
      ...
      await expect(client.getTransactionReceipt('0xinvalid')).rejects.toThrow('Transaction receipt not found')
    })
  })
  ...
  describe('getSourceLocators', () => {
    ...
    it('should fetch source locators for a workload ID', async () => {
      ...
      const result = await client.getSourceLocators(workloadId as `0x${string}`)
    ...
    it('should retry on transient failures', async () => {
      ...
      const mockReadContractWithRetry = clientWithRetry.getClient().readContract as Mock

      mockReadContractWithRetry.mockRejectedValueOnce(new Error('Network error')).mockResolvedValueOnce(mockMetadata)

      const result = await clientWithRetry.getSourceLocators(workloadId as `0x${string}`)
    ...
    it('should throw NetworkError after max retries', async () => {
      ...
      await expect(clientWithRetry.getSourceLocators(workloadId as `0x${string}`)).rejects.toThrow(NetworkError)
      expect(mockReadContractWithRetry).toHaveBeenCalledTimes(2)
    })
  })
