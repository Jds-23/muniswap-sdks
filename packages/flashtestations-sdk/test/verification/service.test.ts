...
import type { WorkloadMeasurementRegisters } from '../../src/types'
...
describe('verifyFlashtestationInBlock', () => {
  let mockgetFlashtestationEvent: Mock
  let mockGetBlock: Mock
  let mockRpcClientConstructor: ReturnType<typeof vi.spyOn>
  let mockComputeAllWorkloadIds: ReturnType<typeof vi.spyOn>
  let mockGetBlockExplorerUrl: ReturnType<typeof vi.spyOn>

  beforeEach(() => {
    // Setup mocks
    mockgetFlashtestationEvent = vi.fn()
    mockGetBlock = vi.fn()

    // Mock RpcClient constructor
    mockRpcClientConstructor = vi.spyOn(rpcClientModule, 'RpcClient').mockImplementation(
      () =>
        ({
    ...
    mockComputeAllWorkloadIds = vi.spyOn(workloadModule, 'computeAllWorkloadIds')

    // Mock chain config functions
    mockGetBlockExplorerUrl = vi
  ...
  describe('with workload ID string', () => {
    ...
    it('should return isBuiltByExpectedTee: true when workload ID matches', async () => {
      const mockEvent = {
        ...
        sourceLocators: [
          'https://github.com/flashbots/flashbots-images/commit/b7c707667393cc4c0173786ee32ec3a79009b04f',
        ],
      }
      ...
      const result = await verifyFlashtestationInBlock(workloadId, 'latest', config)
    ...
    it('should normalize workload ID with 0x prefix', async () => {
      ...
      const result = await verifyFlashtestationInBlock(workloadIdWithoutPrefix, 'latest', config)
    ...
    it('should handle case-insensitive workload ID comparison', async () => {
      ...
      const result = await verifyFlashtestationInBlock(workloadIdUpperCase, 'latest', config)
    ...
    it('should propagate BlockNotFoundError', async () => {
      mockgetFlashtestationEvent.mockRejectedValue(new BlockNotFoundError('latest'))

      await expect(verifyFlashtestationInBlock(workloadId, 'latest', config)).rejects.toThrow(BlockNotFoundError)
    })

    it('should propagate NetworkError', async () => {
      mockgetFlashtestationEvent.mockRejectedValue(new NetworkError('Connection failed'))

      await expect(verifyFlashtestationInBlock(workloadId, 'latest', config)).rejects.toThrow(NetworkError)
    })
  })
  ...
  describe('with WorkloadMeasurementRegisters', () => {
    ...
    it('should compute workload ID from registers and verify', async () => {
      ...
      const result = await verifyFlashtestationInBlock(registers, 'latest', config)
    ...
    it('should compute workload ID from registers and verify when registers have a different case', async () => {
      const uppercaseRegisters: WorkloadMeasurementRegisters = {
        tdattributes: `0x${(registers.tdattributes as string).substring(2).toUpperCase()}`,
        xfam: `0x${(registers.xfam as string).substring(2).toUpperCase()}`,
        mrtd: `0x${(registers.mrtd as string).substring(2).toUpperCase()}`,
        mrconfigid: `0x${(registers.mrconfigid as string).substring(2).toUpperCase()}`,
        rtmr0: `0x${(registers.rtmr0 as string).substring(2).toUpperCase()}`,
        rtmr1: `0x${(registers.rtmr1 as string).substring(2).toUpperCase()}`,
        rtmr2: `0x${(registers.rtmr2 as string).substring(2).toUpperCase()}`,
        rtmr3: `0x${(registers.rtmr3 as string).substring(2).toUpperCase()}`,
      }
      ...
      const result = await verifyFlashtestationInBlock(uppercaseRegisters, 'latest', config)
    ...
    it('should propagate validation errors from computeAllWorkloadIds', async () => {
      ...
      await expect(verifyFlashtestationInBlock(alteredRegisters, 'latest', config)).rejects.toThrow(
        'Invalid tdattributes: expected 16 hex characters, got 15'
      )
    })
  })
  ...
  describe('RpcClient configuration', () => {
    it('should pass chainId to RpcClient', async () => {
      ...
      await verifyFlashtestationInBlock('0xabc', 'latest', { chainId: 1301 })
    ...
    it('should pass custom rpcUrl to RpcClient', async () => {
      ...
      await verifyFlashtestationInBlock('0xabc', 'latest', { chainId: 1301, rpcUrl: 'https://custom.rpc' })
