...
export function addChainFlags(command: Command): Command {
  ...
  return command
    ...
    .option('-c, --chain-id <id>', 'Chain ID (advanced: use numeric chain ID directly)', Number.parseInt)
...
export function resolveChainConfig(options: ChainOptions): ResolvedChainConfig {
  ...
  if (!options.rpcUrl) {
    ...
    if (!isChainSupported(chainId)) {
      throw new ValidationError(
        `Chain ${chainId} is not supported. Available chains: ${availableChains}. Use --rpc-url to specify a custom RPC URL for unsupported chains.`
      )
    }
  }
