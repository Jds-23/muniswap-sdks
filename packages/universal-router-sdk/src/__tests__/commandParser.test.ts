...
import { MAX_TICK, MIN_TICK, encodeSqrtRatioX96, nearestUsableTick } from '@muniswap/v3-sdk'
import { Actions, Pool, V4Planner } from '@muniswap/v4-sdk'
import { AbiFunction, type Hex } from 'ox'
import { describe, expect, it } from 'vitest'
import { CommandParser, type UniversalRouterCall } from '../utils/commandParser'
...
const TICKLIST = [
  {
    index: nearestUsableTick(MIN_TICK, 10),
    liquidityNet: amount,
    liquidityGross: amount,
  },
  {
    index: nearestUsableTick(MAX_TICK, 10),
    liquidityNet: -amount,
    liquidityGross: amount,
  },
]
...
// ABI for encoding execute calldata (no deadline variant)
...
describe('Command Parser', () => {
                ...
                value: [
                  {
                    ...
                    value: {
                      poolKey: {
                        currency0: USDC.address.toLowerCase(),
                        currency1: WETH9[1]!.address.toLowerCase(),
                        fee: USDC_WETH.poolKey.fee,
                        tickSpacing: USDC_WETH.poolKey.tickSpacing,
                        hooks: USDC_WETH.poolKey.hooks,
                      },
                      zeroForOne: true,
  ...
  for (const test of tests) {
    it(`should parse calldata ${test.input.commands}`, () => {
      ...
      const calldata = AbiFunction.encodeData(executeAbi, [commands as Hex.Hex, inputs as Hex.Hex[]])

      const result = CommandParser.parseCalldata(calldata as string)
