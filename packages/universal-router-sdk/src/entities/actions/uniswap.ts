import {
  ...
  type RouteV2,
  type RouteV3,
  type SwapOptions as RouterSwapOptions,
  type Trade as RouterTrade,
  encodeMixedRouteToPath,
  getOutputOfPools,
  partitionMixedRouteByProtocol,
} from '@muniswap/router-sdk'
import type { TPool } from '@muniswap/router-sdk'
import { type Currency, type CurrencyAmount, Percent, type Token, TradeType } from '@muniswap/sdk-core'
import { Pair, Trade as V2Trade } from '@muniswap/v2-sdk'
import { Pool as V3Pool, Trade as V3Trade, encodeRouteToPath } from '@muniswap/v3-sdk'
import {
  Actions,
  V4Planner,
  Pool as V4Pool,
...
export class UniswapTrade implements Command {
  ...
  get outputRequiresWrap(): boolean {
    ...
    if (lastPool instanceof V4Pool) {
      if (!this.trade.outputAmount.currency.isNative) {
        ...
        if (lastPool.currency1.equals(lastPool.currency0.wrapped) && lastRoute.pools.length > 1) {
          const poolBefore = lastRoute.pools[lastRoute.pools.length - 2]!
          if (
          ...
          if (poolBefore.token0.equals(lastPool.currency1) || poolBefore.token1.equals(lastPool.currency1)) {
            return true
          }
        }
      }
  ...
  encode(planner: RoutePlanner, _config: TradeConfig): void {
    ...
    let minimumAmountOut = BigInt(this.trade.minimumAmountOut(this.options.slippageTolerance).quotient.toString())
    if (routerMustCustody) {
      ...
      if (this.options.fee) {
        ...
        if (this.trade.tradeType === TradeType.EXACT_OUTPUT) {
          minimumAmountOut = minimumAmountOut - (minimumAmountOut * BigInt(feeBips)) / 10000n
        }
      }

      if (this.options.flatFee) {
        const feeAmount = this.options.flatFee.amount
        if (minimumAmountOut < feeAmount) throw new Error('Flat fee amount greater than minimumAmountOut')

        planner.addCommand(CommandType.TRANSFER, [pathOutputCurrencyAddress, this.options.flatFee.recipient, feeAmount])

        if (this.trade.tradeType === TradeType.EXACT_OUTPUT) {
          minimumAmountOut = minimumAmountOut - feeAmount
        }
      }
...
function addV4Swap<TInput extends Currency, TOutput extends Currency>(
  ...
  let pathOutputForTake: Currency = trade.route.pathOutput
  const lastPool = v4Route.pools[v4Route.pools.length - 1]!
  const ethWethPool = lastPool.currency1.equals(lastPool.currency0.wrapped)

  if (ethWethPool && v4Route.pools.length > 1) {
    const poolBefore = v4Route.pools[v4Route.pools.length - 2]!
...
function addMixedSwap<TInput extends Currency, TOutput extends Currency>(
  ...
  let inputToken: Currency = route.pathInput
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (routePool instanceof V4Pool) {
      ...
      v4Planner.addSettle(
        inputToken,
        payerIsUser && i === 0,
        (i === 0 ? amountIn : CONTRACT_BALANCE) as unknown as bigint
      )
      ...
      let outputTokenForTake: Currency = outputToken
      if (isLastSectionInRoute(i)) {
        const lastPool = route.pools[route.pools.length - 1]!
        const v4Pool = lastPool instanceof V4Pool
        const ethWethPool = v4Pool
          ? (lastPool as V4Pool).currency1.equals((lastPool as V4Pool).currency0.wrapped)
          : false
        const poolBefore = route.pools[route.pools.length - 2]!
