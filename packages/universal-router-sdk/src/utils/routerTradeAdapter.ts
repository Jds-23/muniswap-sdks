import { MixedRouteSDK, Trade as RouterTrade } from '@muniswap/router-sdk'
import { type Currency, CurrencyAmount, Ether, Token, type TradeType } from '@muniswap/sdk-core'
import { Pair, Route as V2Route } from '@muniswap/v2-sdk'
import { type FeeAmount, Pool as V3Pool, Route as V3Route } from '@muniswap/v3-sdk'
import { Pool as V4Pool, Route as V4Route } from '@muniswap/v4-sdk'
...
export class RouterTradeAdapter {
  ...
  private static toCurrency(isNative: boolean, token: TokenInRoute): Currency {
    ...
    return RouterTradeAdapter.toToken(token)
  }
  ...
  private static toToken(token: TokenInRoute): Token {
    ...
    return new Token(
      chainId,
      address,
      Number.parseInt(decimals.toString()),
      symbol,
      /* name */ undefined,
      false,
  ...
  private static toV3Pool({ fee, sqrtRatioX96, liquidity, tickCurrent, tokenIn, tokenOut }: V3PoolInRoute): V3Pool {
    return new V3Pool(
      ...
      Number.parseInt(fee) as FeeAmount,
      sqrtRatioX96,
      liquidity,
      Number.parseInt(tickCurrent)
    )
  }
  ...
  private static toV4Pool(pool: V4PoolInRoute): V4Pool {
    ...
    return new V4Pool(
      parsedCurrencyIn,
      parsedCurrencyOut,
      Number.parseInt(pool.fee) as FeeAmount,
      Number.parseInt(pool.tickSpacing),
      pool.hooks,
      pool.sqrtRatioX96,
      pool.liquidity,
      Number.parseInt(pool.tickCurrent)
    )
  }
