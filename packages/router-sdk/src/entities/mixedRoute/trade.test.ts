...
describe('MixedRouteTrade', () => {
  ...
  function v2StylePool(
    ...
    const tick = getTickAtSqrtRatio(sqrtRatioX96)
  ...
  describe('is backwards compatible with pure v3 routes', () => {
    ...
    describe('#bestTradeExactIn', () => {
      it('throws with empty pools', async () => {
        await expect(
          MixedRouteTrade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, BigInt(10000)), token2)
  ...
  describe('is backwards compatible with pure v2 routes', () => {
    ...
    describe('#bestTradeExactIn', () => {
      it('throws with empty pairs', async () => {
        await expect(
          MixedRouteTrade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, BigInt(100)), token2)
        ).rejects.toThrow('POOLS')
      })
      it('throws with max hops of 0', async () => {
        await expect(
          MixedRouteTrade.bestTradeExactIn([pair_0_2], CurrencyAmount.fromRawAmount(token0, BigInt(100)), token2, {
            maxHops: 0,
          })
        ).rejects.toThrow('MAX_HOPS')
      })
    ...
    describe('#fromRoutes', () => {
      ...
      it('can be constructed with ETHER as output for exact input with multiple routes', async () => {
        const trade = await MixedRouteTrade.fromRoutes<Token, Ether, TradeType.EXACT_INPUT>(
          [
            {
              amount: CurrencyAmount.fromRawAmount(token1, BigInt(3000)),
              route: new MixedRouteSDK([pair_0_1, pool_v3_weth_0], token1, ETHER),
            },
            {
              amount: CurrencyAmount.fromRawAmount(token1, BigInt(7000)),
