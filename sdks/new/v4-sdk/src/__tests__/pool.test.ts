import { ChainId, CurrencyAmount, Ether, Token, WETH9 } from '@uniswap/sdk-core-next'
import { MAX_TICK, MIN_TICK, encodeSqrtRatioX96, getTickAtSqrtRatio, nearestUsableTick } from '@uniswap/v3-sdk-next'
import { beforeEach, describe, expect, it } from 'vitest'
import { Pool } from '../entities/pool'
import { DYNAMIC_FEE_FLAG } from '../internalConstants'
import {
  ADDRESS_ZERO,
  FEE_AMOUNT_HIGHEST,
  FEE_AMOUNT_LOW,
...
import { constructHookAddress } from './testUtils'
...
describe('Pool', () => {
  ...
  describe('#v4InvolvesToken', () => {
    it('works with native currency pool', () => {
      const pool = new Pool(
    ...
    it('works with wrapped currency pool', () => {
  ...
  describe('#bigNums', () => {
    ...
    describe('#priceLimit', () => {
      ...
      it('correctly handles two BigInts', async () => {
        const inputAmount = CurrencyAmount.fromRawAmount(USDC, 100)
        const [outputAmount] = await pool.getOutputAmount(inputAmount)
