import { type Currency, Price } from '@uniswap/sdk-core-next'
...
export class Route<TInput extends Currency, TOutput extends Currency> {
  ...
  public constructor(pools: Pool[], input: TInput, output: TOutput) {
    ...
    // If the input is native and the first pool is an eth-weth pool, that means we already wrapped the input
    ...
    this.input = input
    this.output = output ?? (currencyPath[currencyPath.length - 1] as TOutput)
  }
